...
3. 按提示输入DeepSeek API Key（输入时不会显示，确保保密）

4. 等待依赖安装完成，FastAPI服务器将在端口8000启动

#### 前端启动

1. 打开终端2，进入frontend目录：
```bash
cd frontend
```
...
3. 等待依赖安装完成，Vite开发服务器将在端口5173启动
...
1. 在开场白区域查看RIPER-5模式说明和高考信息模板
2. 复制模板内容到输入框，填写个人信息
3. 点击"发送"按钮或按Enter键发送消息
4. 查看AI回复是否包含 `[MODE: RESEARCH]` 标签
5. 验证消息显示格式：头像、时间戳、对齐方式
6. 测试空消息提示、加载状态、错误处理等功能
